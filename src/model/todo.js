...
/**
 * @typedef TodoEntry
 * @propert (Boolean)  {string}  title - текст записи
 * @property [completed] - флаг завершенности задачи
 ...
 * @param  (Boolean) Object} query - критерии поиска todo для удаления
...
async function deleteTodo (query) {
  ...
  /*
    ...
    Подсказка: используйте поле `result` из результата выполнения функции col.deleteOne,
 ...
 * @param {Object} query - критерии поиска todo
 * @param  (Boolean) Object} data - атрибуты todo для обновления
 * @returns - `true` - если запись в базе данных была обновлена
...
async function createTodosFromText (filePath, email) {
  ...
  const col = dbConnection.getCollection(COLLECTION)
  const insertedIds = await col.insertMany(todos.map(todo => ({
    ...todo, email
  })))

  return insertedIds.ops
  /*
